...
-- This script creates the minimum tables needed for the admin dashboard to work
...
CREATE POLICY IF NOT EXISTS "Allow all operations" ON public.agencies FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow all operations" ON public.properties FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow all operations" ON public.proposals FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow all operations" ON public.invite_codes FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow all operations" ON public.activity_logs FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow all operations" ON public.notifications FOR ALL USING (true);
CREATE POLICY IF NOT EXISTS "Allow all operations" ON public.contracts FOR ALL USING (true);
...
-- Update user_profiles to link with the default agency
DO $$
DECLARE
    default_agency_id UUID;
BEGIN
    SELECT id INTO default_agency_id FROM public.agencies WHERE name = 'Atria Imobiliária' LIMIT 1;
    
    -- Add agency_id column to user_profiles if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_profiles' AND column_name = 'agency_id') THEN
        ALTER TABLE public.user_profiles ADD COLUMN agency_id UUID REFERENCES public.agencies(id);
    END IF;
    
    -- Update existing users to link with default agency
    UPDATE public.user_profiles SET agency_id = default_agency_id WHERE agency_id IS NULL;
END $$;
